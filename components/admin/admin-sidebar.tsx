'use client';

import React from 'react';
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { LiveCalicutLogo } from '@/components/shared/live-calicut-logo';
import {
  LayoutDashboard,
  Users,
  Store,
  Newspaper,
  Calendar,
  Briefcase,
  ShoppingBag,
  Building,
  Flag,
  FolderTree,
  MapPin,
  Settings,
  ShieldCheck,
  ArrowLeft,
  CreditCard,
  BarChart3,
  LayoutTemplate,
} from 'lucide-react';

export const AdminSidebar: React.FC = () => {
  const pathname = usePathname();

  const navGroups = [
    {
      group: 'OVERVIEW',
      items: [
        { label: 'Control Center', href: '/admin', icon: LayoutDashboard },
        { label: 'Platform Analytics', href: '/admin/analytics', icon: BarChart3 },
        { label: 'Audit Trail Logs', href: '/admin/audit-logs', icon: ShieldCheck },
      ],
    },
    {
      group: 'ECOSYSTEM MANAGEMENT',
      items: [
        { label: 'Users & Roles', href: '/admin/users', icon: Users },
        { label: 'Commercial Outlets', href: '/admin/businesses', icon: Store },
        { label: 'Cyberpark Jobs', href: '/admin/jobs', icon: Briefcase },
        { label: 'Classifieds Market', href: '/admin/marketplace', icon: ShoppingBag },
        { label: 'Real Estate Listings', href: '/admin/properties', icon: Building },
      ],
    },
    {
      group: 'CITY CONTENT & MEDIA',
      items: [
        { label: 'Landing Page CMS', href: '/admin/cms', icon: LayoutTemplate },
        { label: 'News & Editorial', href: '/admin/news', icon: Newspaper },
        { label: 'Cultural Events', href: '/admin/events', icon: Calendar },
        { label: 'Categories Taxonomy', href: '/admin/categories', icon: FolderTree },
        { label: 'Cities & Wards', href: '/admin/cities', icon: MapPin },
      ],
    },
    {
      group: 'GOVERNANCE & FINANCE',
      items: [
        { label: 'Moderation Flags', href: '/admin/reports', icon: Flag },
        { label: 'Billing & Payments', href: '/admin/billing', icon: CreditCard },
        { label: 'System Settings', href: '/admin/settings', icon: Settings },
      ],
    },
  ];

  return (
    <aside className="w-64 shrink-0 bg-white border-r border-[#E5E7EB] min-h-screen p-4 space-y-6 flex flex-col justify-between shadow-xs">
      <div className="space-y-6">
        {/* Admin Brand Logo Header */}
        <div className="px-2 py-2 border-b border-[#E5E7EB] pb-4">
          <div className="flex items-center gap-2">
            <LiveCalicutLogo showSubtitle={false} />
          </div>
          <div className="mt-2.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-[10px] font-extrabold text-[#2563EB] tracking-wide uppercase font-sans">
            <ShieldCheck className="w-3 h-3 text-[#2563EB]" />
            <span>Admin OS v3.2</span>
          </div>
        </div>

        {/* Grouped Sidebar Navigation */}
        <div className="space-y-6">
          {navGroups.map((group) => (
            <div key={group.group} className="space-y-1.5">
              <h5 className="px-3 text-[10px] font-extrabold text-[#9CA3AF] tracking-widest uppercase font-sans">
                {group.group}
              </h5>
              <nav className="space-y-0.5">
                {group.items.map((item) => {
                  const Icon = item.icon;
                  const isActive = pathname === item.href;

                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-bold transition-all ${
                        isActive
                          ? 'bg-[#2563EB] text-white shadow-md shadow-blue-500/20'
                          : 'text-[#4B5563] hover:text-[#111827] hover:bg-[#F8FAFC]'
                      }`}
                    >
                      <Icon className="w-4 h-4 shrink-0" />
                      <span className="truncate">{item.label}</span>
                    </Link>
                  );
                })}
              </nav>
            </div>
          ))}
        </div>
      </div>

      {/* Exit Control Center Shortcut */}
      <div className="pt-4 border-t border-[#E5E7EB]">
        <Link
          href="/"
          className="flex items-center gap-2 px-3 py-2 text-xs text-[#6B7280] hover:text-[#111827] font-bold hover:bg-[#F8FAFC] rounded-xl transition-all"
        >
          <ArrowLeft className="w-4 h-4 text-[#2563EB]" />
          <span>Exit to Public Portal</span>
        </Link>
      </div>
    </aside>
  );
};
